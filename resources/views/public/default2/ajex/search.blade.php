...
<div class="rock-event">
	...
	@if(!empty($aet->event_image))
  		@if(is_numeric($aet->event_image)) 	 
   			<?php 
   			/*Query for Image of Event*/
   			$evtoimg = DB::table('event_catimages')
   								->where('id', '=', $aet->event_image)
   								->select('id','ecat_name','ecat_path','ecat_image')
   								->get(); 
   			?>	
    		
    		@if(!empty($evtoimg[0]->id))	
     		<img src="{!!URL::to('uploads/'.$evtoimg[0]->ecat_path.'/'.$evtoimg[0]->ecat_name.'/'.$evtoimg[0]->ecat_image)!!}" 
	...
	<div class="evnt-con col-md-9 col-ms-7 col-xs-12">
		<h4>
 			<a href="{!!URL('event/'.$aet->event_url)!!}" 
 				class="delegation-ajax" 
 				data-tipped-options="ajax:{data:{ event: '{!! $aet->event_url !!}', _token: '{{ csrf_token() }}' } }" 
 				target="_blank"> 
        ...
        <div class="date"> <span class="date-icon"> </span>
			...
			if(!empty($aet->country)){	 
			    $getvcy 	= DB::table('countries')
			    				->where('id', '=', $aet->country)
			    				->select('name')
			    				->get(); 
		...
		<!-- People Attending Data -->
        <div class="attend">
	        <?php $allevt_ct = DB::table('users_events')->where('e_id', '=', $aet->id)->count(); ?>	
			People Attending : <span class="blue-col"> {!! $allevt_ct !!} </span>
		</div>	
