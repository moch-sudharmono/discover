<div class="full pop-design">
 <div class="map-evnt home-map" id="event-map{!!$event_data[0]->eid!!}"> </div>
<script>  
...
<meta property="og:title" content="{!!ucfirst($event_data[0]->event_name)!!}"> 
		...
		<div class="col-md-4 col-sm-4 text-center round-pic">
		 <img class="img-responsive" src="{!!URL::to($event_img)!!}"/>        
        </div>
...
<?php } ?> 
 ...
 <div class="gry-bg"> 	
	...
	<a href="mailto:someone@example.com?Subject=discoveryourevent+website+forum" target="_top" class="social-fb">
		<img src="{!!URL::to('assets/public/default2/images/mail-to.png')!!}"/>
	 </a>
	 <!----
...
<script> 
var vtsml{!!$event_data[0]->eid!!} = ['<?php echo implode('","', $maddArr);?>'];
var mrgitctitle{!!$event_data[0]->eid!!} = ['<?php echo ucfirst($event_data[0]->event_name);?>'];
 setTimeout(function() { mcitcaddress(vtsml{!!$event_data[0]->eid!!}[0],mrgitctitle{!!$event_data[0]->eid!!}[0],'event-map{!!$event_data[0]->eid!!}'); },1000);
</script>