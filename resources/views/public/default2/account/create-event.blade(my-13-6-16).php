      ...
      <div class="tabbable tabs-left">
       @include("public/default2/_layouts._AccountMenu")
		 ...
		 <h3>Create Your Event</h3>
		  ...
		  @if(count($errors) > 0 || !empty(Session::get('share_event')))
		  ...
		  @endif
		   ...
		   @if(!empty(Session::get('inv_tkurl')) || !empty(Session::get('ev_date')) || !empty(Session::get('ev_section')))
			<span class="col-md-12 full-error alert">
             <button data-dismiss="alert" aria-label="close" class="close">&times;</button>
             <strong>Whoops!</strong> There were some problems with your input.
            </span>	  
		   @endif	 	    
		  @if(!empty(Session::get('req_evprice')) || !empty(Session::get('failed_upfile')) || !empty(Session::get('pev_req')))
			<span class="col-md-12 full-error alert">
             <button data-dismiss="alert" aria-label="close" class="close">&times;</button>
             <strong>Whoops!</strong> There were some problems with your input.
            </span>	
			...
			<div class="col-lg-12 col-sm-6 col-xs-12 row margin-b10">
			 <label class="width-set"> <span class="require-val">*</span>Starts: </label>			
			  <div class="input-group date custom-dtime padd-n-xs" id="datetimepicker2"> 
                  <input type="text" name="sevent_date" class="form-control" value="{{ old('sevent_date') }}"/>
...
<!-----multi-date----->	
  <div class="new-period"></div>
   ...
   <div class="edit-se_multiple form-group col-lg-12 col-sm-12 col-xs-12">
   ...
   <div class="edit-series-schedule-monthly-day" id="essmd-monthly-day" style="display:none">
	<h3>It occurs every</h3>
	  <select id="edit-series-schedule-monthly-day">
    ...
    <div class="form_add_event series-schedule-weekly">
	   <h3>What day(s) of the week?</h3>
	  <select id="edit-series-schedule-weekly" class="js-series-weekly-weekdays" multiple="multiple">	
	   <?php 
  ...
  <h4>From</h4>					 
 <div class="input-group date custom-dtime padd-n-xs">			 
  <select class="time-set" name="timestart" id="edit-start-time" onchange="tedcimecounts();">
   <?php 
  ...
  <h4>To</h4>					 
 <div class="input-group date custom-dtime padd-n-xs">			 
  <select class="time-set" name="timeend" id="edit-end-time" onchange="tedcimecounts();">
   <?php 
    foreach($tmformat as $tmfkey => $tmfval){
	 echo '<option value="'.$tmfkey.'">'.$tmfval.'</option>';   
    }
   ...
   <h4>of the</h4>
  <select id="edit-schd_event_seires" class="schedule_event_seires" onchange="tedcimecounts();">	
   <?php 
   ...
   ?> 
	...
	<div class="date-range-section">
	 <div class="ouur_form_head">
		<h4 id="edit-custom_change">Occurs from</h4>		
		<input type="text" id="edit_arrdatepicker" />
	 </div>
	 <div class="until-datesection" id="edit-until-datesection">
		<h4>Until</h4>
		<input type="text" id="edit_depdatepicker" />
	 </div>	
	</div>	
 ...
 <div class="start-end_multiple form-group col-lg-12 col-sm-12 col-xs-12" style="display: none;">
  <div class="heading">
  ...
  <div class="form_add_event series-schedule-weekly">
	 <h3>What day(s) of the week?</h3>
	<select id="ul-series-schedule-weekly" class="js-series-weekly-weekdays" multiple="multiple">
     <?php 
	  foreach($sdwkfmt as $sdfkey => $sdfval){
		echo '<option value="'.$sdfkey.'">'.$sdfval.'</option>'; 
	  }
   ...
   <div class="series-schedule-monthly-day">
	 <h3>It occurs every</h3>
	<select id="series-schedule-monthly-day">
	 <option value="">Month Date</option>
	 <?php 
		foreach($month_darray as $mdkey => $mdval){
		 echo  '<option value="'.$mdkey.'">'.$mdval.'</option>'; 
 ...
 <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
  ...
  <div class="form-group col-lg-12 col-sm-12 col-md-12 col-xs-12"> 
   <div class="schedule_event_div col-lg-6 col-sm-12 col-md-6 col-xs-12">
   ...
   var secat_url = secat.replace(/ /g, "_");
	...
	$('.pubprv').click(function() {
     $('input.pubprv').not(this).prop('checked', false); 
	  if($('input#prv-evntjs').is(":checked"))
     {
       $('#private_option').show();   	
	 } else {
	   $('#private_option').hide();  
	...
	$('.sip_event').click(function() { 
     $('input.sip_event').not(this).prop('checked', false); 
    });	
	...
	$('#restlocation').click(function() {
	 ...
	 function updateLvsControls(addressComponents) {
		$('#ev_address').val(addressComponents.addressLine1);
