...
class EventController extends Controller {
    ...
    public function getMevent() {
        ...
        if (Sentry::check()) {
            ...
            $all_event = DB::table('events')
                    ...
                    ->select(   'id', 'account_id', 'account_type', 'event_name', 
                                'event_url', 'event_image', 'event_type', 
                                'event_dtype', 'event_date', 'event_cost', 'event_venue')
                    ->paginate(10);
        ...
        if (Sentry::check()) {
            ...
            if (!empty($uaccount_dts[0]->city)) {
                $all_events = DB::table('events')
                                ...
                                ->select(   'id', 'event_name', 'account_id', 
                                            'account_type', 'event_url', 'event_image', 
                                            'event_venue', 'event_address', 'address_secd', 
                                            'city', 'state', 'country', 
                                            'event_dtype', 'event_date', 'event_time', 
                                            'event_cost', 'event_price')
                ...
                $all_events = DB::table('events')
                                    ...
                                    ->select(   'id', 'event_name', 'account_id', 
                                                'account_type', 'event_url', 'event_image', 
                                                'event_venue', 'event_address', 'address_secd', 
                                                'city', 'state', 'country', 'event_dtype', 
                                                'event_date', 'event_time', 'event_cost', 'event_price')
                                    ->orderBy('id', 'desc')
                                    ->paginate(8);
            }
            $womicon = 8;
            ...
            if (!empty($event_data[0]->eid)) {
                ...
                if (!empty($event_data[0]->state) && !empty($event_data[0]->country)) {
                    ...
                    $getssdata = DB::table('states')
                                    ->where('country_id', '=', $event_data[0]->country)
                                    ->where('name', '=', $event_data[0]->state)
                                    ->select('id', 'name')
                                    ->get();
                ...
                return \View::make('public/default2/ajex/pevent-ajex')
                            ->with('event_data', $event_data)
                            ->with('maddArr', $maddressArray)
                            ->with('attending_people', $count_eattpeopl)
                            ->with('your_event', $ur_event)
    ...
    public function accEventpopup($eventurl) 
    {
        ...
        if (!empty($eventurl)) {
            ...
            if (!empty($event_data[0]->id)) {
                ...
                return \View::make('public/default2/ajex/account_event')
                            ->with('event_data', $event_data)
                            ->with('attending_people', $count_eattpeopl)
                            ->with('your_event', $ur_event);
    ...
    public function postAccevent($aname, Request $request) {
        ...
        if (!empty($accountdetail[0]->g_id)) {
            ...
            if ($validator->fails()) {
                ...
                $av_purchase        = (isset($data['available_purchase'])?'y':'n');
                $kid_event          = (isset($data['kid_event'])?'y':'n');
                $family_event       = (isset($data['family_event'])? 'y':'n');
                $religious_event    = (isset($data['religious_event'])?'y':'n');
                ...
                if ($data['event_dtype'] == 'single') {
                    ...
                    if (empty($data['sevent_date'])) {
                        ...
                        return Redirect::back()
                                ->with('ev_date', 'Event Start date field is required')
                                ->withInput();
                        die;
                    ...
                    if (empty($data['arrdate'])) {
                        ...
                        return Redirect::back()
                                ->with('ev_section', 'Event date Section is required')
                                ->withInput();
                        die;
                ...
                if ($data['private_event'] == 'y') {
                    ...
                    if (!isset($data['sharepinvt_event']) && empty($data['sharepinvt_event'])) {
                        ...
                        return Redirect::back()
                                ->with('share_event', 'Please choose one option, i\'ts required')
                                ->withInput();
                        die();
                    ...
                    if ($share_event == 'n') {
                        ...
                        if (isset($password_estatus) && $password_estatus == 'y') {
                            ...
                            if (empty($pass_event)) {
                                ...
                                return Redirect::back()
                                        ->with('pev_req', 'Private event password field is required')
                                        ->withInput();
                                die;
                ...
                if (!empty($input_file['event_image'][0])) {
                    ...
                    if ($event_filesize < 2000001) {
                        ...
                        if ($mime_type == 'image/png' || $mime_type == 'image/gif' || $mime_type == 'image/jpeg' || $mime_type == 'image/jpg' || $mime_type == 'image/bmp') {
                            ...
                            /**************resize-image*************** */
                            $nw_evid = DB::table('events')
                                        ->insertGetId([ 'u_id'                  => $clid, 
                                                        'account_id'            => $accountdetail[0]->id, 
                                                        ...
                                                        'fb_link'               => $data['fb_link'],
                                                        'tw_link'               => $data['tw_link'], 
                                                        'available_purchase'    => $av_purchase, 
                                                        'kid_event'             => $kid_event, 
                                                        'family_event'          => $family_event,
                                                        'religious_event'       => $religious_event, 
                                                        'private_event'         => $data['private_event'], 
                            ...
                            return Redirect::back()
                                    ->with('failed_upfile', 'Image type not supported, please upload (png/gif/jpeg/jpg/bmp) format')
                                    ->withInput();
                            die;
                ...
                if (!empty($accountdetail[0]->id)) {
                    ...
                    /**********mail************ */
                    $memail_data    = DB::table('mail_notifications')
                                        ->where('yfollow_page', '=', 'y')
                                        ->select('id', 'user_id', 'yfollow_page')
                                        ->get();
                    ...
                    if ($memail_data[0]->yfollow_page == 'y') {
                        ...
                        foreach ($memail_data as $med_uid) {
                            ...
                            $mud_data = DB::table('users')
                                            ->where('id', '=', $med_uid->user_id)
                ...
                return Redirect::to($aname . '/account/manageEvent')
                        ->with('succ_mesg', 'Event created successfully');
    ...
    public function postAccupdatevent($aname, $eid, Request $request) 
    {
        ...
        $accountdetail = DB::table('account_details')
                            ->where('account_details.u_id', '=', $clid)
                            ->where('account_details.account_url', '=', $aname)
                            ...
                            ->select('account_details.id', 'account_details.g_id')
                            ->get();
            ...
            if ($validator->fails()) {
                ...
                if ($data['private_event'] == 'y') {
                    ...
                    if (!isset($data['sharepinvt_event']) && empty($data['sharepinvt_event'])) {
                        ...
                        return Redirect::back()
                                ->with('share_event', 'Please choose one option, its required')
                                ->withInput();
                        die();
                    ...
                    foreach ($data['arrdate'] as $stdt) {
                        ...
                        if (isset($data['multieid'][$xlx]) && !empty($data['multieid'][$xlx])) {
                            ...
                            DB::table('event_multidate')
                                ->where('id', '=', $data['multieid'][$xlx])
                                ->where('ev_id', '=', $eid)
                                ->update([  'event_datetype'=> $data['nspd'][$xlx], 
                                            'start_date'    => $startdt, 
                                            'enddate'       => $itcendate, 
                                            'ofthe'         => $data['ofthe'][$xlx],
                                            'start_time'    => $data['startime'][$xlx], 
    ...
    public function saveEvent($eventurl) 
    {
        if (Sentry::check() && !empty($eventurl)) {
            ...
            $check_data = DB::table('events')
                            ->where('user_evdelete', '=', 'n')
                            ->where('event_url', '=', $eventurl)
                            ->select('id', 'u_id', 'account_id', 'event_name', 'event_url')
                            ->get();

            if (!empty($check_data[0]->id)) {

                $check_esaved = DB::table('users_events')
                                    ->where('e_id', '=', $check_data[0]->id)
                                    ->where('u_id', '=', $clid)
                                    ->select('id')
                                    ->get();
                ...
                if (!empty($check_esaved[0]->id)) {
                    ...
                    return Redirect::to('event/' . $eventurl)
                            ->with('succ_mesg', 'Event already added on your account');

                } else {

                    DB::table('users_events')
                        ->insertGetId([ 'e_id'          => $check_data[0]->id, 
                                        'u_id'          => $clid, 
                                        'eowner_id'     => $check_data[0]->u_id, 
                                        'created_at'    => date('Y-m-d H:i:s'), 
                                        'updated_at'    => date('Y-m-d H:i:s')
                                    ]);
                    ...
                    DB::table('notifications')
                        ->insert([  'onr_id'        => $check_data[0]->u_id, 
                                    'u_id'          => $clid, 
                                    'type'          => 'event', 
                                    'subject'       => $notfsub, 
                                    'object_id'     => $check_data[0]->id, 
                                    'object_type'   => 'attend', 
                                    'created_at'    => date('Y-m-d H:i:s'), 
                                    'updated_at'    => date('Y-m-d H:i:s')
                                ]);
                    ...
                    if (!empty($check_data[0]->account_id)) {
                        ...
                        $account_data = DB::table('account_details')
                                            ->where('account_details.id', '=', $check_data[0]->account_id)
                                            ->join('users', 'account_details.u_id', '=', 'users.id')
                                            ->select('account_details.id', 'account_details.name', 'account_details.account_url', 'account_details.flemail_update', 'users.full_name', 'users.email')
                                            ->get();
                    ...
                    if (empty($check_data[0]->account_id)) {
                        ...
                        $mailevent_data = DB::table('mail_notifications')
                                            ->where('mail_notifications.user_id', '=', $check_data[0]->u_id)
                                            ->join('users', 'mail_notifications.user_id', '=', 'users.id')
                                            ->select('mail_notifications.id', 'mail_notifications.event_attend', 'users.full_name', 'users.email')
                                            ->get();
    ...
    public function unattendEvent(Request $request) 
    {
        ...
        if (Sentry::check()) {
            ...
            $chk_uent = DB::table('users_events')
                            ->where('e_id', '=', $data['evntid'])
                            ->where('u_id', '=', $uid)
                            ->select('id', 'eowner_id')
                            ->get();
            ...
            if (!empty($chk_uent[0]->id)) {
                ...
                DB::table('notifications')
                    ->insert([  'onr_id'        => $chk_uent[0]->eowner_id, 
                                'u_id'          => $uid, 
                                'type'          => 'event', 
                                'subject'       => $notfsub, 
                                'object_id'     => $data['evntid'], 
                                'object_type'   => 'unattend', 
                                'created_at'    => date('Y-m-d H:i:s'), 
                                'updated_at'    => date('Y-m-d H:i:s')
                            ]);
                ...
                if (!empty($event_data[0]->account_id)) {
                    $account_data = DB::table('account_details')
                                        ->where('account_details.id', '=', $event_data[0]->account_id)
                                        ->join('users', 'account_details.u_id', '=', 'users.id')
                                        ->select('account_details.id', 'account_details.name', 'account_details.account_url', 'account_details.flemail_update', 'users.full_name', 'users.email')
                                        ->get();
                ...
                if (empty($event_data[0]->account_id)) {
                    $mailevent_data = DB::table('mail_notifications')
                                        ->where('mail_notifications.user_id', '=', $event_data[0]->u_id)
                                        ->join('users', 'mail_notifications.user_id', '=', 'users.id')
                                        ->select('mail_notifications.id', 'mail_notifications.event_attend', 'users.full_name', 'users.email')
                                        ->get();
            ...
            if (isset($refine_events[0]) && !empty($refine_events[0]->id)) {
                ...
                foreach ($refine_events as $flce) {
                    ...
                    if (!empty($flce->state) && !empty($flce->country)) {
                        $getssdata = DB::table('states')
                                        ->where('country_id', '=', $flce->country)
                                        ->where('name', '=', $flce->state)
                                        ->select('id', 'name')
                                        ->get();
    ...
    {		
        ...
        if (Sentry::check() && !empty($page)) {
            ...
            return \View::make('public/default2/ajex/ajex-rsearch')
                    ->with('refine_events', $refine_events)
                    ->with('cust_class', $custom_class)
                    ->with('clid', $lg_id);
    ...
    public function postEvntfollow(Request $request) 
    {
        if (Sentry::check()) {
            ...
            if (!empty($get_ownerid[0]->u_id)) {
                $cflw = DB::table('user_follows')
                            ->where('follow_id', '=', $data['ascid'])
                            ->where('u_id', '=', $uid)
                            ->where('follow_type', '=', 'event')
                            ->select('id')
                            ->get();

                if (empty($cflw[0]->id)) {
                    DB::table('user_follows')
                        ->insert([  'follow_id'     => $data['ascid'], 
                                    'u_id'          => $uid, 
                                    'owner_id'      => $get_ownerid[0]->u_id,
                                    'follow_type'   => 'event', 
                                    'created_at'    => date('Y-m-d H:i:s'), 
                                    'updated_at'    => date('Y-m-d H:i:s')
                                ]);
    ...
    {        
        ...
        if (!empty($data['eurl'])) {
            $event_data = DB::table('events')
                            ->where('events.user_evdelete', '=', 'n')
                            ->where('events.event_url', '=', $data['eurl'])
                            ->select('events.id', 'events.contact_person', 'events.email_address', 'events.event_name', 'events.event_image', 'events.event_date', 'events.event_time', 'events.end_date', 'events.end_time', 'events.event_description', 'events.fb_link', 'events.tw_link')
                            ->get();
    ...
    public function getEventimg($gname) {
        ...
        if (!empty($gname)) {
            ...
            $ecimg = DB::table('event_catimages')
                        ->where('ecat_name', 'LIKE', $fldname . '%')
                        ->get();
