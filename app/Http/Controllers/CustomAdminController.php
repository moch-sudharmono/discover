...
public function eventList()
{
	$all_event = DB::table('events')
					->join('users', 'events.u_id', '=', 'users.id')
					->select(	'events.id','events.event_name','users.full_name',
								'events.account_type','events.event_image','events.event_date',
								'events.event_cost','events.private_event','events.event_url','events.event_block')
					->orderBy('events.id', 'desc')
					->get();

   return \View::make('backend/default/events/show')
   				->with('all_event', $all_event)
   				->with('title','Event List - DiscoverYourEvent')
   				->with('active','event');
}
...
{ 
	if(!empty($evid) && !empty($bl)){	
	  ...
	  $gevuid = DB::table('events')
	  			->where('events.id', '=', $evid)
	  			->join('users', 'events.u_id', '=', 'users.id')
	  			->select('events.u_id','events.event_name','users.id','users.full_name','users.email')
	  			->get();

	   DB::table('events')
	   		->where('id', '=', $evid)
	   		->update(['event_block' => $bl,'updated_at' => date('Y-m-d H:i:s')]);
       ...
       $cmntf = DB::table('mail_notifications')
       				->where('user_id', '=', $gevuid[0]->id)
       				->select('enotification_status')
       				->get();			
	    ...
	    if($cmntf[0]->enotification_status == 'y'){
      	...
      	return Redirect::to('backend/eventList')
      			->with('succ_mesg', 'Event updated successfully');
...
public function eventReview($evid)
{
	if(!empty($evid)){	
		$single_event = DB::table('events')
							->where('id', '=', $evid)->get();
	  	return \View::make('backend/default/events/review')
	  				->with('sing_event', $single_event)
	  				->with('title','Event Review - DiscoverYourEvent')
	  				->with('active','event');
...
public function getStats()
{
	...
	return \View::make('backend/default/stats')
				->with('title','Statistics - DiscoverYourEvent')
				->with('wvcount', $pv_count)
				->with('eventCount', $allEventc)
				->with('drcount',$drcount)
				->with('rucount',$rucount)
				->with('unvistr',$unvist)
				->with('acc_count', $alladc)
				->with('active','stats');		
} 

public function getStatGraph()
{
	$getval = DB::table('unique_visits')
				->select(DB::raw('id,date, count(*) as alldc, count(distinct IP) as dip'))
	 			->orderBy('date')
	 			->groupBy('date')
	 			->get(); 
