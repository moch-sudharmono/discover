...
/**************************User-Profile************************************/	
	...
	if(Sentry::check()){
		...
		if(!empty($data['evntsdata'])){
	   		$unfl_acc = DB::table('account_details')
	   						->where('u_id', '!=', $uid)
	   						->where('name', 'like', '%'.$data['evntsdata'].'%')
	   						->select('id','name','account_url')
	   						->orderBy('name','ASC')
	   						->get();

     		if(!empty($unfl_acc[0]->id)){
	  			foreach($unfl_acc as $unflw_acc){
       				$unfep = DB::table('user_follows')
       							->where('u_id', '=', $uid)
       							->where('follow_id', '=', $unflw_acc->id)
       							->distinct('follow_id')
       							->select('id')
       							->get();   
	...
	if(Sentry::check()){
		...
		if(!empty($data['evtsdata'])){
	  		if($data['evtsdata'] == 'a'){	  
       			$unfl_acc = DB::table('users_events')
       							->where('users_events.u_id', '=', $uid)
       							->where('events.user_evdelete', '=', 'n')
       							->join('events', 'users_events.e_id', '=', 'events.id')
	  							->select(	'events.id','events.event_name','events.account_id',
	  										'events.account_type','events.event_url','events.event_image',
	  										'events.event_venue','events.event_address','events.event_date',
	  										'events.event_time','events.event_cost','events.event_price')
     		...
     		if(!empty($unfl_acc[0]->id)){
				...
				echo $asdata = '<div class="evnt-pic col-md-12 col-ms-12 col-xs-12"><div class="evnt-pic col-md-6 col-ms-6 col-xs-6">Empty data</div></div>';  
...
/**************************User-Account************************************/	
	...
	if(Sentry::check()){
		...
		if ($validator->fails()) {
		  ...
		  $c_exitun 	= DB::table('account_details')
			  				->where('id', '!=', $data['acc_id'])
			  				->where('name', '=', $data['name'])
			  				->select('id')
			  				->get();	

		  $aul_exitun 	= DB::table('account_details')
		  					->where('id', '!=', $data['acc_id'])
		  					->where('account_url', '=', $data['account_url'])
		  					->select('id')
		  					->get();	
			...
			if(!empty($c_exitun[0]->id) || !empty($aul_exitun[0]->id)){
		 		if(!empty($c_exitun[0]->id)){
		  			return Redirect::back()
		  					->with('name_error', 'The name has already been taken.')
		  					->withInput();	 
		 		}	 

		 		if(!empty($aul_exitun[0]->id)){
		  			return Redirect::back()
		  					->with('acurl_error', 'The Account URL has already been taken.')
		  					->withInput();	 
		 		}		  
         	} else {			 
				$input_file 	= $request->file(); 
		  		$get_adtls 		= DB::table('account_details')
		  							->where('id', '=', $data['acc_id'])
		  							->select('id','g_id','upload_file')
		  							->get();	
	...
	if(Sentry::check()){
		$uid 	= Sentry::getUser()->id; 
       	...
       	$get_ownerid = DB::table('account_details')
       					->where('id', '=', $data['ascid'])
       					->select('id','u_id','name','account_url','flemail_update')
       					->get();
     	...
     	if(!empty($get_ownerid[0]->u_id)){	   
	 		...
	 		if(empty($cflw[0]->id)){ 
	  			...
	  			DB::table('notifications')
	  				->insert([	'onr_id' 		=> $get_ownerid[0]->u_id,
	  							'u_id' 			=> $uid,
	  							'type' 			=> 'page',
	  							'subject' 		=> $notfsub,
	 							'object_id' 	=> $data['ascid'],
	 							'object_type'	=> 'follow',
	 							'created_at' 	=> date('Y-m-d H:i:s'),
	 							'updated_at' 	=> date('Y-m-d H:i:s')
	 						]);
	...
	if(Sentry::check()){
		$uid 		= Sentry::getUser()->id; 
       	...
       	$check_flws = DB::table('user_follows')
       						->where('id', '=', $data['flwid'])
       						->where('u_id', '=', $uid)
       						->select('id','follow_id','owner_id')
       						->get();	  

     	if(!empty($check_flws[0]->id)){	   
	  		DB::table('user_follows')
	  			->where('id', '=', $check_flws[0]->id)
	  			->update([	'follow' => 'n', 
	  						'updated_at' => date('Y-m-d H:i:s')
	  					]);	

	  		$get_ownerid = DB::table('account_details')
	  						->where('id', '=', $check_flws[0]->follow_id)
	  						->select('id','u_id','name','account_url','flemail_update')
	  						->get();
	  		...
	  		DB::table('notifications')
	  			->insert([	'onr_id' 		=> $check_flws[0]->owner_id,
	  						'u_id' 			=> $uid,
	  						'type' 			=> 'page',
	  						'subject' 		=> $notfsub,
	 						'object_id' 	=> $check_flws[0]->follow_id,
	 						'object_type'	=> 'follow',
	 						'created_at' 	=> date('Y-m-d H:i:s'),
	 						'updated_at' 	=> date('Y-m-d H:i:s')
	 					]);
  			...
  			if(!empty($get_ownerid[0]->id)){
	 			...
	 			$onrinfo = DB::table('users')
	 						->where('id', '=', $get_ownerid[0]->u_id)
